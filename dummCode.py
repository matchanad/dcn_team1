...
def handle(update: Update) -> None:
    ...
    try:
        ...
        if command == '/setup_new_user':
            if current_chat_id is None:
                current_chat_id = chat_id
                chat_states[chat_id] = STATE_IDLE
                bot.send_message(chat_id=chat_id, text='User setup complete. You can now use the other commands.')
            else:
                bot.send_message(chat_id=chat_id, text='This device is already set up by another user.')
            return

        if chat_id != current_chat_id:
            bot.send_message(chat_id=chat_id, text='This device is already set up by another user. You cannot use it.')
            return
